...
///
/// The inner bytes are private so wire-format code is the only place that can
/// depend on the representation. `ZERO` exists solely as an internal
/// uninitialized/fallback value and is rejected by packet decoding.
...
/// preserving the exact four-byte wire representation.
