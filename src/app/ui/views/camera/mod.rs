...
use crate::{camera, display::Display, theme};
...
impl View {
    ...
    pub(crate) fn present_shell(&self, display: &mut Display) {
        display.render_scanlines(design::CONTENT_REGION, |_local_y, pixels| {
            pixels.fill(theme::BLACK_RGB565);
        });
    }

    pub(crate) fn render(&self, display: &mut Display, frame: &mut camera::Frame<'_>) {
