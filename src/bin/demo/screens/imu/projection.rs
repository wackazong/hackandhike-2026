...
use hack_and_hike::capabilities::imu::Orientation;
...
pub(super) fn display_attitude(orientation: &Orientation) -> DisplayAttitude {
    let gravity = screen_to_camera(orientation.gravity_screen);
    let roll = libm::atan2f(-gravity[0], -gravity[1]).to_degrees();
    let horizontal = libm::sqrtf(gravity[0] * gravity[0] + gravity[1] * gravity[1]);
    let pitch = -libm::atan2f(gravity[2], horizontal).to_degrees();
    DisplayAttitude {
        roll_deg: roll,
        pitch_deg: pitch,
        ...
        basis: WorldBasis {
            gravity_screen: orientation.gravity_screen,
            north_screen: orientation.north_screen,
        },
    }
}
