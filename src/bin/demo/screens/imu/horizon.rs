...
use hack_and_hike::ui::{common, gui::GuiFramebuffer, theme};
...
pub(super) fn draw_attitude(frame: &mut GuiFramebuffer, area: Rect, attitude: DisplayAttitude) {
    ...
    common::fill(frame, area, theme::LIGHT_BLUE);
    ...
    if camera.cos_roll.abs() > HORIZON_VERTICAL_COS_EPSILON {
        ...
        for local_x in 0..width {
            ...
            if camera.cos_roll > 0.0 {
                ...
                common::vline(frame, x0 + local_x, y0, horizon as u32, theme::DARK_GRAY);
            }
        }
    } else {
        for local_x in 0..width {
            ...
            if ground_side {
                common::vline(frame, x0 + local_x, y0, height as u32, theme::DARK_GRAY);
            }
        }
    }
